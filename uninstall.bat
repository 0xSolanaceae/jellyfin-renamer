...
if not exist "%~dp0..\jellyfin-rename.exe" (
    echo.
    powershell -Command "Write-Host '[ERROR] Could not find jellyfin-rename.exe in parent directory!' -ForegroundColor Red"
    powershell -Command "Write-Host 'Expected location: %~dp0..\jellyfin-rename.exe' -ForegroundColor Yellow"
    goto :error
)
...
if not exist "%~dp0..\assets\jellyfin.ico" (
    ...
    powershell -Command "Write-Host 'Expected location: %~dp0..\assets\jellyfin.ico' -ForegroundColor Yellow"
    goto :error
)
...
if not exist "%~dp0menu-option.reg" (
    ...
    powershell -Command "Write-Host 'Expected location: %~dp0menu-option.reg' -ForegroundColor Yellow"
    goto :error
)
...
copy /Y "%~dp0..\jellyfin-rename.exe" "%INSTALL_DIR%\" >nul 2>&1
if %ERRORLEVEL% neq 0 (
    echo.
...
copy /Y "%~dp0..\assets\jellyfin.ico" "%INSTALL_DIR%\" >nul 2>&1
if %ERRORLEVEL% neq 0 (
    echo.
...
regedit /s "%~dp0menu-option.reg" 2>nul
if %ERRORLEVEL% neq 0 (
    echo.
